...
public class SpringAMQPConsumer extends DefaultConsumer implements ConnectionListener {
    ...
    public SpringAMQPConsumer(final SpringAMQPEndpoint endpoint, final Processor processor) {
    ...
    protected static Map<String, Object> parseKeyValues(final String routingKey) {
    ...
    private class RabbitMQMessageListener implements ChannelAwareMessageListener {
        ...
        public RabbitMQMessageListener(final SpringAMQPEndpoint endpoint) {
    ...
    private class SpringAMQPExecutor extends SimpleAsyncTaskExecutor {
        ...
        SpringAMQPExecutor(final SpringAMQPEndpoint endpoint) {
            this.endpoint = endpoint;
        }
    ...
    private class SpringAMQPExecutorTask implements Runnable {
        ...
        public SpringAMQPExecutorTask(final SpringAMQPEndpoint endpoint, final Runnable delegateTask) {
            this.endpoint = endpoint;
        ...
        protected void declareAMQPEntities() {
            if (endpoint.hasDlxName())
                declareAmqpDlxEntities();
            Queue queue = declareQueue();
            if (endpoint.isUsingDefaultExchange()) {
                ...
                org.springframework.amqp.core.Exchange exchange = declareExchange(this.endpoint.createAMQPExchange());
                declareBinding(exchange, queue, this.endpoint.getRoutingKey());
            }
        }

        protected void declareAmqpDlxEntities() {
            org.springframework.amqp.core.Exchange deadLetterExchange = new TopicExchange(endpoint.getDlxName(), true, false);
            deadLetterExchange = declareExchange(deadLetterExchange);
            Queue deadLetterQueue = new Queue(endpoint.getDlqName(), true);
            this.endpoint.getAmqpAdministration().declareQueue(deadLetterQueue);
            declareBinding(deadLetterExchange, deadLetterQueue, "#");
        }

        protected org.springframework.amqp.core.Exchange declareExchange(final org.springframework.amqp.core.Exchange exchange) {
            try {
        ...
        protected Queue declareQueue() {
            ...
            if (endpoint.hasDlxName())
                queueArguments.put(DEAD_LETTER_EXCHANGE_ARGUMENT, endpoint.getDlxName());
        ...
        protected Binding declareBinding(final org.springframework.amqp.core.Exchange exchange, final Queue queue, final String routingKey) {
